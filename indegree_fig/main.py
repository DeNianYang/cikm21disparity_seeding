...
def plotting(action_retrival, action_str, testcase):
    ...
    p1 = figure(
        ...
        x_axis_type='log')
        #,x_axis_label='Number of {}'.format(action_str)
    ...
    if action_str == 'Comment - Indegree': 
        ...
        source = ColumnDataSource(data=dict(x=[1, 1],
                                            y=[1e-2, 1.6e-2],
                                            names=['Female mean: 13.00(±10.58)', 'Male mean: 12.30(±10.15)']))
        ...
        source = ColumnDataSource(data=dict(x=[1, 1],
                                            y=[1e-2, 1.6e-2],
                                            names=['Female mean: 25.30(±15.74)', 'Male mean: 23.27(±17.36)']))
        labels = LabelSet(x='x', y='y', text='names', level='glyph',
              x_offset=5, y_offset=5, source=source, render_mode='canvas',text_font_size="20pt")
        ...
        source = ColumnDataSource(data=dict(x=[1, 1],
                                            y=[1e-2, 1.6e-2],
                                            names=['Female mean: 9.04(±7.76)', 'Male mean: 6.73(±6.84)']))
        labels = LabelSet(x='x', y='y', text='names', level='glyph',
              x_offset=5, y_offset=5, source=source, render_mode='canvas',text_font_size="20pt")
    ...
    if action_str == "Tag - Indegree":
        p1.xaxis.ticker = [10,20,30,50,70]
...
for testcase_idx in range(1):
    ...
    for byType in range(3):
        if byType == 0: action_str = "Comment - Indegree"
        elif byType == 1: action_str = "Like - Indegree"
        elif byType == 2: action_str = "Tag - Indegree"
...
curdoc().add_root(p)
curdoc().title = "glassceiling - Indegree"
