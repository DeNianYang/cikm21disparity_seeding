...
seed100 = ['100']
FILE_READ = 'newfile.csv'
# like and comment
male_spread_list = [[[] for j in range(5)] for i in range(5)]
female_spread_list = [[[] for j in range(5)] for i in range(5)]
...
# like and comment
# ratio and spread
data_retrival = [[[[] for j in range(5)] for i in range(2)] for k in range(3)]
...
def plotting(action_retrival, action_str):
    p1 = figure(
        ...
        # y_range=(0, 1),
        x_range=['0.0','0.2','0.4','0.8','1.0'],y_range=(0,1),
        y_axis_label='Spread Ratio (Female)',
    ...
    data = {'x': idx1, 'y': y1, 'radius': [y/1000 for y in x1]}
    data1 = {'x': idx2, 'y': y2, 'radius': [y/1000 for y in x2]}
    data2 = {'x': idx3, 'y': y3, 'radius': [y/1000 for y in x3]}
    data3 = {'x': idx4, 'y': y4, 'radius': [y/1000 for y in x4]}
    data4 = {'x': idx5, 'y': y5, 'radius': [y/1000 for y in x5]}
    #data5 = {'x': idx6, 'y': y6, 'radius': [y/18e4 for y in x6]}
    ...
    data = {'x_axis' : ['0.0','0.2','0.4','0.8','1.0'],
        'error' : [0, 0.2-PF, 0.4-PF, 0.8-PF, 1.0-PF],
        'margin' : [PF, 2*PF, 2*PF, 2*PF, PF]}


    p1.vbar_stack(['error', 'margin'], x='x_axis', width=0.9, color=['rgba(0, 0, 0, 0)', "gray"], source=data, alpha=0.35, legend_label='Error Margins ±{}%'.format(round(PF*100)))

    
    
    p1.circle(x='x', y='y', radius='radius', color='red', alpha=0.5, source=source)
    p1.line(idx1, y1, line_width=7, color='red', line_dash="4 4")
    
    p1.circle(x='x', y='y', radius='radius', color='black', alpha=0.5, source=source1)
    p1.line(idx2, y2, line_width=7, color='black', line_dash="4 4")

    p1.circle(x='x', y='y', radius='radius', color='green', alpha=0.5, source=source2)
    p1.line(idx3, y3, line_width=7, color='green', line_dash="4 4")

    p1.circle(x='x', y='y', radius='radius', color='yellow', alpha=0.5, source=source3)
    p1.line(idx4, y4, line_width=7, color='yellow', line_dash="4 4")

    p1.circle(x='x', y='y', radius='radius', color='purple', alpha=0.5, source=source4)
    p1.line(idx5, y5, line_width=7, color='purple', line_dash="4 4")

    #p1.circle(x='x', y='y', radius='radius', color='purple', alpha=0.5, source=source5)
    #p1.line(idx6, y6, line_width=5, color='purple', line_dash="4 4")
    ...
    #p1.varea(x=['0.0','0.2','0.4','0.8','1.0'], y1=[0, 0.2,0.4,0.8,1.0], y2=[0+PF, 0.2+PF, 0.4+PF, 0.8+PF, 1.0+PF], legend_label='Error Margins ±{}%'.format(round(PF*100)), alpha=0.25, color='gray')
    '''
    p1.varea(x=[0.2, 0.4], y1=[PF, 1+PF], y2=[-PF, 1-PF], alpha=0.25, color='gray')
    p1.varea(x=[0., 1], y1=[PF, 1+PF], y2=[-PF, 1-PF], alpha=0.25, color='gray')
    p1.varea(x=[0, 1], y1=[PF, 1+PF], y2=[-PF, 1-PF], alpha=0.25, color='gray')
    p1.varea(x=[0, 1], y1=[PF, 1+PF], y2=[-PF, 1-PF], alpha=0.25, color='gray')
    '''
    ...
    export_svgs(p1, filename="fig_new/parento_frontier_{}_{}_100_new.svg".format(action_str, PF))
    return p1, paras2save

plots = {'like': {}, 'comment': {}, 'tag':{}}
paras = [{},{},{}]
for byType in range(3):
    if byType == 0: action_str = "Like"
    elif byType == 1: action_str = "Comment"
