...
# like and comment
male_spread_list = [[[] for j in range(4)] for i in range(4)]
female_spread_list = [[[] for j in range(4)] for i in range(4)]

for algo in range(4):
    #if algo == 0: algo_str = 'none'
    if algo == 0: algo_str = 'hiindex'
    elif algo == 1: algo_str = 'pagerank'
    elif algo == 2: algo_str = 'frequency'
    elif algo == 3: algo_str = 'agnostict'
            ...
            if int(token[2]) == 5000:
                if algo_str in token[1]:
                    ...
                    if algo == 5:
                        ...
                        ratio_sub = token[1].split('-')[3].split('.')
                        ratio = float(ratio_sub[0]+"."+ratio_sub[1])
                        if token[0] == 'comment':
                            #print("fefefe")
                            if algo_str in token[1]:
                                #print(algo_str)
                                female[0][int(ratio*10)] = float(token[3])
                                male[0][int(ratio*10)] = float(token[4])
                        elif token[0] == 'like':
                            #print("fefefe")
                            if algo_str in token[1]: 
                                #print(algo_str)
                                female[1][int(ratio*10)] = float(token[3])
                                male[1][int(ratio*10)] = float(token[4])
...
data_retrival = [[[[] for j in range(4)] for i in range(2)] for k in range(2)]
...
def plotting(action_retrival, action_str):
    ...
    #ata4 = {'x': idx5, 'y': y5, 'radius': [y/5000 for y in x5]}
    ...
    p1.line(idx3, y3, line_width=5, color='orange', line_dash="4 4")

    #p1.circle(x='x', y='y', radius='radius', color='yellow', alpha=0.5, source=source3)
    p1.line(idx4, y4, line_width=5, color='black', line_dash="4 4")
    ...
    export_svgs(p1, filename="fig_ig/ig_new_parento_frontier_{}_{}_5000.svg".format(action_str, PF))
