...
class studentDataset(object):
    ...
    def _load(self):
        ...
        edges_all = np.genfromtxt("../only_post/students_edges_like.csv",
                                        dtype=np.int32,
                                        delimiter=',')
        ...
        for i in range(edges_all.shape[0]):
            ...
            if edge not in edge_dict:
                edge_dict[edge] = weight[i]
                indegree_list.append(edges_all[i][1])
        """
        node_num = self.graph.number_of_nodes()
        for i in range(node_num):
            if i not in indegree_list:
                edge
        """

        self.weight = edge_dict
