...
class studentDataset(object):
    ...
    def _load(self):
        ...
        idx_features = np.genfromtxt("../../../OpenNE/fb_tag_emb.txt", dtype=np.dtype(str), delimiter=' ')
        #print(idx_features)
        idx_labels = np.genfromtxt("../only_post/students_labels.csv",dtype=np.dtype(str),delimiter=' ')
        #print(idx_labels)
        features = sp.csr_matrix(idx_features[:,1:],dtype=np.float32)
        real_idx = list(idx_features[:,0])
        #print(real_idx)
        features_copy = features
        for idx in range(len(real_idx)):
            features[int(real_idx[idx])] = features_copy[idx]
        #print(idx_labels[:,-1])
        labels = _encode_onehot(idx_labels[:,-1])
        self.num_labels = labels.shape[1]
        #print(labels)
        # build graph
        idx = np.asarray(idx_labels[:, 0], dtype=np.int32)
