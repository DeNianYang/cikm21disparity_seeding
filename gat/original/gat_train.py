...
class CrossEntropyLoss(object):
    ...
    def get_loss(self):
        ...
        anchor = self.p_dst
        #print(anchor)
        positive = nn.MSELoss(self.g.ndata['s'], self.g.ndata['s_est'])
...
class GATLayer(nn.Module):
    ...
    def edge_attention(self, edges):
        ...
        b_sum = torch.exp(self.attn_fc(z_sum))




        return {'e': torch.mul(F.leaky_relu(a), add), 'e_sum': b_sum}

    def message_func(self, edges):
        # message UDF for equation (3) & (4)
        return {'z': edges.src['z'], 'e': edges.data['e'], 'e_sum' = edges.data['e_sum']}
    ...
    def reduce_func(self, nodes):
        ...
        #compute newer part
        combine = F.softmax(nodes.mailbox['e_sum'],dim=1) * s
        self.g.ndata['combine'] = combine
...
for _ in range(avg_loop):
    ...
    OUTPUT_DIR = 'influence_net_comment_{}.csv'.format(_)
