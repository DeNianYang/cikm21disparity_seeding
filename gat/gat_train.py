...
class PositiveSampler(object):
    ...
    def p_samples(self, g, node_num):
        ...
        sg = dgl.sampling.sample_neighbors(g, src, self.k, edge_dir='out', prob=g.edata['intensity'],replace=True)
        return sg.edges()
...
class CrossEntropyLoss(object):
    ...
    def get_loss(self):
        ...
        #print(anchor)
        mse_loss = nn.MSELoss()
        positive = mse_loss(self.g.ndata['s'], self.g.ndata['s_est'])
        #print(positive.size())
        ...
        loss2 = (torch.sum(positive) - torch.sum(negative))
        
        mse_loss = nn.MSELoss()
        #loss3 = mse_loss(s_0, self.g.ndata['s'])
        loss3 = torch.norm(g.ndata['s'], 0)
        #print(loss1, loss2, loss3)

        loss4 = torch.norm(g.ndata['emb'], 2)

        return loss1  + loss2 * self.weight1 + loss3 * self.weight2 + loss4 * self.weight3
...
class GATLayer(nn.Module):
    def __init__(self, g, in_dim, out_dim):
        ...
        self.new_fc = nn.Linear(2 * in_dim, 1, bias=False)
        #influence loss
        self.inf_fc = nn.Linear(in_dim,1,bias=False)
    ...
    def edge_attention(self, edges):
        ...
        z_sum = torch.cat([edges.src['h'], edges.dst['h']], dim=1)
        #print(z_sum.size())
        e_sum = self.new_fc(z_sum)

        #combine = F.softmax(nodes.mailbox['e'], dim=1)





        return {'e': torch.mul(F.leaky_relu(a), add), 'e_sum': e_sum}

    def message_func(self, edges):
        # message UDF for equation (3) & (4)
        return {'z': edges.src['z'], 'e': edges.data['e'], 'e_sum':edges.data['e_sum'], 's' : edges.src['s']}
    ...
    def reduce_func(self, nodes):
        ...
        temp = F.softmax(nodes.mailbox['e_sum'],dim=1)
        s_est = torch.sum(temp * nodes.mailbox['s'],dim=1)

        
        #lower = torch.sum(nodes.mailbox['e_sum'] * nodes.mailbox['s'],dim=1)

        #compute newer part
        #s = self.g.ndata['s']
        #combine = F.softmax(nodes.mailbox['e_sum'],dim=1) 
        

        #s_est = torch.sum(nodes.mailbox['combine'], dim=1)

        


        return {'h': h, 's_est': s_est}

    def forward(self, h):
        # equation (1)
        z = self.fc(h)
        s = self.inf_fc(h)
        self.g.ndata['s'] = torch.sigmoid(s)
        self.g.ndata['z'] = z
        self.g.ndata['h'] = h
        # equation (2)
...
from gat_preprocess_new import studentDataset
...
for _ in range(avg_loop):
    ...
    for epoch in range(200):
        ...
        #print(p_src, p_dst)
        #print(n_src, n_dst)
        loss_obj = CrossEntropyLoss(g, p_src, p_dst,n_src, n_dst, s_0, 1e-4, 2, 5)
    ...
    for i in range(g.number_of_nodes()):
        ...
        score = torch.tensor([0.0])
    '''
    indegrees = g.in_degrees()

    for i in range(g.number_of_nodes()):
        gat_score_list[i] += float(emb_tensor[i] * float(indegrees[i])) * 1.0/avg_loop
        #score = torch.tensor([0.0])
    ...
    OUTPUT_DIR = 'pretrain/influence_net_tag_sig.csv'
