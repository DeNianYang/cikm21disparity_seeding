            ...
            if "comment" in edge_type:
                ...
                if usr1 in gender_dict:
                    if usr2 in gender_dict:
                        ...
                        if usr2 not in edge_list[0]:
                            edge_list[0][usr2] = [usr1]
                            outdegree_list[0][usr2] = 1
                        else:
                            if usr1 not in edge_list[0][usr2]:
                                edge_list[0][usr2].append(usr1)
                                outdegree_list[0][usr2] += 1
                ...
                if usr1 in gender_dict:
                    if usr2 in gender_dict:
                        ...
                        if usr2 not in edge_list[1]:
                            edge_list[1][usr2] = [usr1]
                            outdegree_list[1][usr2] = 1
                        else:
                            if usr1 not in edge_list[1][usr2]:
                                edge_list[1][usr2].append(usr1)
                                outdegree_list[1][usr2] += 1
                ...
                if usr1 in gender_dict:
                    if usr2 in gender_dict:
                        ...
                        if usr2 not in edge_list[2]:
                            edge_list[2][usr2] = [usr1]
                            outdegree_list[2][usr2] = 1
                        else:
                            if usr1 not in edge_list[2][usr2]:
                                edge_list[2][usr2].append(usr1)
                                outdegree_list[2][usr2] += 1
...
line2write = []
OUTPUT_DIR = 'indegree/students_edges_all.csv'
...
for i in range(len(post_type)):
    sorted_out_degree_list[i] = sorted(outdegree_list[i].items(), key = lambda d : d[1], reverse = True)



for i in range(3):
    for pair in sorted_out_degree_list[i]:
        usr = pair[0]
        outdegree_num = pair[1]
        line2write.append((id_dict[usr], gender_dict[usr], outdegree_num))

    #insert 0
    for j in id_dict:
        if j not in outdegree_list[i]:
            line2write.append((id_dict[j], gender_dict[j], 0))

    OUTPUT_DIR = 'outdegree/students_outdegree_{}.csv'.format(types[i])
    with open(OUTPUT_DIR, 'w') as writefile:
                writer = csv.writer(writefile)
                for line in line2write:
                    writer.writerow(line)
