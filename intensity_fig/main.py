...
def plotting(action_retrival, action_str, testcase):
    ...
    if action_str == 'Comment - Intensity': 
        source = ColumnDataSource(data=dict(x=[1, 1],
                                            y=[1e-2, 1.6e-2],
                                            names=['Female mean: 1036.66(±1439)', 'Male mean: 736.73(±1174.8)']))
    ...
    elif action_str == 'Like - Intensity': 
        source = ColumnDataSource(data=dict(x=[1, 1],
                                            y=[1e-2, 1.6e-2],
                                            names=['Female mean: 882.49(±1398.9)', 'Male mean: 632.15(±1199.2)']))
        labels = LabelSet(x='x', y='y', text='names', level='glyph',
              x_offset=5, y_offset=5, source=source, render_mode='canvas',text_font_size="20pt")
    ...
    elif action_str == 'Tag - Intensity': 
        source = ColumnDataSource(data=dict(x=[0.7, 0.7],
                                            y=[1e-2, 1.6e-2],
                                            names=['Female mean: 203.57(±539)', 'Male mean: 96.79(±392)']))
        labels = LabelSet(x='x', y='y', text='names', level='glyph',
              x_offset=5, y_offset=5, source=source, render_mode='canvas',text_font_size="20pt")
...
for testcase_idx in range(1):
    ...
    for byType in range(3):
        if byType == 0: action_str = "Comment - Intensity"
        elif byType == 1: action_str = "Like - Intensity"
        elif byType == 2: action_str = "Tag - Intensity"
...
curdoc().add_root(p)
curdoc().title = "glassceiling - Intensity"
