...
def plotting(action_retrival, action_str, testcase):
    ...
    if action_str == 'Comment - HI-index': 
        source = ColumnDataSource(data=dict(x=[0.95, 0.95],
                                            y=[1.3e-2, 1.8e-2],
                                            names=['Female mean: 9.67(±6.77)', 'Male mean: 9.04(±6.88)']))
    ...
    elif action_str == 'Like - HI-index': 
        source = ColumnDataSource(data=dict(x=[0.95,0.95],
                                            y=[1e-2, 1.6e-2],
                                            names=['Female mean: 19.31(±10.72)', 'Male mean: 17.54(±11.72)']))
        labels1 = LabelSet(x='x', y='y', text='names', level='glyph',
              x_offset=5, y_offset=5, source=source, render_mode='canvas',text_font_size="19pt")
    ...
    elif action_str == 'Tag - HI-index': 
        source = ColumnDataSource(data=dict(x=[0.95, 0.95],
                                            y=[1e-2, 1.6e-2],
                                            names=['Female mean: 5.33(±4.71)', 'Male mean: 4.15(±4.31)']))
        labels1 = LabelSet(x='x', y='y', text='names', level='glyph',
              x_offset=5, y_offset=5, source=source, render_mode='canvas',text_font_size="19pt")
    ...
    if action_str == "Comment - HI-index":
        p1.xaxis.ticker = [5,10,15,20,30,40,50]
    elif action_str == "Like - HI-index":
        p1.xaxis.ticker = [5,10,20,30,40,60]
    elif action_str == "Tag - HI-index":
        p1.xaxis.ticker = [5,10,15,20,30]
...
for testcase_idx in range(1):
    ...
    for byType in range(3):
        if byType == 0: action_str = "Comment - HI-index"
        elif byType == 1: action_str = "Like - HI-index"
        elif byType == 2: action_str = "Tag - HI-index"
...
curdoc().add_root(p)
curdoc().title = "glassceiling - HI-index"
